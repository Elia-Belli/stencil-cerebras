...
star_coefficients = [
  np.array([0.25,0.25,-1,0.25,0.25], dtype=np.float32),
  np.array([0.0625, 0.0625, 0.0625, 0.0625, -1.0, 0.0625, 0.0625, 0.0625, 0.0625], dtype=np.float32),
  np.array([0.0625,0.0625,0.125,0.0625,0.0625,0.125,-1.0625,0.125,0.0625,0.0625,0.125,0.0625,0.0625], dtype=np.float32)
]
...
def read_args():
  ...
  return args, verify, suppress_traces
...
def generate_input(M, N, shape="random"):
  ...
  elif(shape == "tri-diagonal"):
    A = tridiagonal_input(M, N)

  return A
  
def tridiagonal_input(M, N):
  pass
