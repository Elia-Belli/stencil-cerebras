...
def cpu_stencil_3r(A, m, n, c, iters):
  y = A.ravel()
  ...
  for _ in range(iters):
    ...
    for i in range(m):
      for j in range(n):
        idx = i*n +j
        # center
        y_aux[idx] = c[6] *y[i*n+j]
        
        # north
        if(i-3 >= 0): y_aux[idx] += c[0] * y[(i-3)*n+j]
        if(i-2 >= 0): y_aux[idx] += c[1] * y[(i-2)*n+j]
        if(i-1 >= 0): y_aux[idx] += c[2] * y[(i-1)*n+j]
        # south
        if(i+3 < m) : y_aux[idx] += c[12] * y[(i+3)*n+j]
        if(i+2 < m) : y_aux[idx] += c[11] * y[(i+2)*n+j]
        if(i+1 < m) : y_aux[idx] += c[10] * y[(i+1)*n+j]

        # west
        if(j-3 >= 0) : y_aux[idx] += c[3] * y[i*n+j-3]
        if(j-2 >= 0) : y_aux[idx] += c[4] * y[i*n+j-2]
        if(j-1 >= 0) : y_aux[idx] += c[5] * y[i*n+j-1]
        # east
        if(j+3 < m) : y_aux[idx] += c[9] * y[i*n+j+3]
        if(j+2 < m) : y_aux[idx] += c[8] * y[i*n+j+2]
        if(j+1 < n) : y_aux[idx] += c[7] * y[i*n+j+1]
