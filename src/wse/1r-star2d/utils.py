...
def cpu_stencil_1r(A, m, n, iters):
  y = A.ravel()
...
def cpu_stencil_2r(A, m, n, iters):
  y = A.ravel()
  y_aux = np.zeros(m*n, dtype=np.float32)
  ...
  for _ in range(iters):
    for i in range(m):
      for j in range(n):
        if(i-2 >= 0): # up
          y_aux[i*n+j] += y[(i-1)*n+j]
          y_aux[i*n+j] += y[(i-2)*n+j]
        if(i+2 < m) : # down
          y_aux[i*n+j] += y[(i+1)*n+j]
          y_aux[i*n+j] += y[(i+2)*n+j]
        if(j-2 >= 0): # left
          y_aux[i*n+j] += y[i*n+j-1]
          y_aux[i*n+j] += y[i*n+j-2]
        y_aux[i*n+j] -= 4.0*y[i*n+j]  # center
        if(j+2 < n) : #right
          y_aux[i*n+j] += y[i*n+j+1]
          y_aux[i*n+j] += y[i*n+j+2]
